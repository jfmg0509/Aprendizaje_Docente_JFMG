...
func TestAPICreateUser(t *testing.T) {
	// Repos en memoria (solo lo mínimo para probar API)
	ur := newMemUserRepo()
	br := newMemBookRepo()
	ar := newMemAccessRepo()

	// Cola (aunque sea mínima)
	...
	// OJO: para este test solo pegamos a /api/*, así que renderer puede ser nil
	h := NewHandler(userSvc, bookSvc, nil)
	router := NewRouter(h)
...
//
// --------------------
// Repos en memoria (mínimos para tests)
// --------------------
//
...
func (r *memUserRepo) List(ctx context.Context) ([]*domain.User, error) { return []*domain.User{}, nil }
...
func (r *memBookRepo) List(ctx context.Context) ([]*domain.Book, error) { return []*domain.Book{}, nil }
