...
func (h *Handlers) UIUsers(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { renderError(w, h.tpl, err); return }
...
func (h *Handlers) APIUpdateUser(w http.ResponseWriter, r *http.Request) {
    ...
    name := ""
    if in.Name != nil { name = *in.Name }
    email := ""
    if in.Email != nil { email = *in.Email }
    role := domain.Role("")
    if in.Role != nil { role = *in.Role }
...
func (h *Handlers) APIDeleteUser(w http.ResponseWriter, r *http.Request) {
    ...
    id := mustID(w, r)
    if id == 0 { return }
    if err := h.users.Delete(ctx, id); err != nil { writeErr(w, err); return }
...
func (h *Handlers) APIDeleteBook(w http.ResponseWriter, r *http.Request) {
    ...
    id := mustID(w, r)
    if id == 0 { return }
    if err := h.books.Delete(ctx, id); err != nil { writeErr(w, err); return }
    writeJSON(w, http.StatusOK, map[string]any{"deleted": id})
}
